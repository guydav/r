...
ps0 <- try(matchit(treat ~ x , data = dt, distance = "logit"))
...
if((class(mod1)!="try-error") & (class(ps1)!="try-error") & (class(ps2)!="try-error")){ 
...
# naive pscore
ps0 <- try(matchit(treat ~ age + education + black + married + nodegree + re74 + re75 +
...
### R code from vignette source 'MSE-tabs'
...
print(file="MSEtab1.tex", xtable(mat, display=c("s","f","f","f","f","f","f","d")), type="latex", floating=FALSE, sanitize.text.function = function(x){x})

mat1 <- mat
for(i in 1:NROW(mat)) mat1[i,-7] <- mat[i,-7]/mat["PS0",-7]
mat1 <- mat1[-(1:2),]

pdf("MSE1D.pdf", width=9,  height=3.5)
par(mar=c(3,3,1,1))
...
print(file="MSESUBtab1.tex",  xtable(mat, display=c("s","f","f","f","f","f","f","d")), type="latex", floating=FALSE, sanitize.text.function = function(x){x})

pdf("MSE1DCH.pdf", width=9,  height=3.5)
...
print(file="MSEtab2.tex", xtable(mat, display=c("s","f","f","f","f","f","d")), type="latex", floating=FALSE, sanitize.text.function = function(x){x})
...
pdf("MSE5D.pdf", width=9,  height=3.5)
...
colnames(mat) <- c(sprintf("$g_%d$",1:5),"$m_T$")
rownames(mat) <- toupper(rownames(mat))
print(file="MSESUBtab2.tex", xtable(mat,display=c("s","f","f","f","f","f","d")), type="latex", floating=FALSE, sanitize.text.function = function(x){x})
...
pdf("MSE5DCH.pdf", width=9,  height=3.5)
...
rownames(mat) <- toupper(rownames(mat))
print(file="MSEtab3.tex", xtable(mat,display=c("s","f","f","f","f","d")), type="latex", floating=FALSE, sanitize.text.function = function(x){x})
...
pdf("MSELL.pdf", width=9,  height=3.5)
...
colnames(mat) <- c(sprintf("$g_%d$",1:4),"$m_T$")
rownames(mat) <- toupper(rownames(mat))
print(file="MSESUBtab3.tex", xtable(mat,display=c("s","f","f","f","f","d")), type="latex", floating=FALSE, sanitize.text.function = function(x){x})
...
pdf("MSELLCH.pdf", width=9,  height=3.5)
